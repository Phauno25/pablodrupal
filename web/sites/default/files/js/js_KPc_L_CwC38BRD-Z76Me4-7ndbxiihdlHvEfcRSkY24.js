...
window.addEventListener("load", e => {
    ...
    let data = fetch("https://jsonplaceholder.typicode.com/users")
        ...
        .then(res => {
            ...
            displayFilas(res, $('#tablaMascota tbody'), filasPorPagina, paginaActual);
            setearPaginacion(res, $('#paginador'), filasPorPagina);

            $('#formMascotas').on('submit', ev => {
                ev.preventDefault();
                filtrar(res);
            });
            /* es.forEach(e => {
                ...
                tr.innerHTML = `<td>${e.id}</td> \
                    ...
                    <td>${tipoCaso[Math.floor(Math.random() * tipoCaso.length)]}</td>\
                    <td></td>\
                    <td></td>\
                    <td></td>\
                    <td></td>\
            ...
            $('#tablaMascota tbody').append(fragment)  */

        })
})

function filtrar(data) {
    $('tablaMascota tbody').empty();
    const tipoAnimal = $("#tipoMascota").val().toUpperCase();
    const razaAnimal = $("#razaMascota").val().toUpperCase();
    const sexoAnimal = $("#sexoMascota").val().toUpperCase();
    const dataFiltrada = [];

    data.forEach(item => {
        if (data.tipoAnimal.toUpperCase() == tipoAnimal || tipoAnimal == "ALL") {
            if (data.tipoAnimal.toUpperCase() == razaAnimal || razaAnimal == "ALL") {
                if (data.sexoAnimal.toUpperCase() == sexoAnimal || sexoAnimal == "ALL") {
    ...
    displayFilas(dataFiltrada,$('#tablaMascota tbody'),filasPorPagina,paginaActual);
    setearPaginacion(dataFiltrada, $('#paginador'), filasPorPagina);
}

function filtrarNombreMascota() {

    nombreMascota = $("#nombreMascota").val().toUpperCase();
    ...
    $('tbody tr').each(e => {
        nombreVal = e.$('td')[3].textContent || e.$('td')[3].innerText;
        tipoVal = e.$('td')[4].textContent || e.$('td')[4].innerText;
        sexoVal = e.$('td')[5].textContent || e.$('td')[5].innerText;
        razaVal = e.$('td')[6].textContent || e.$('td')[6].innerText;

        if (nombreVal.toUpperCase().indexOf(nombreMascota) > -1 || nombreVal == "All") {
            e.style.display = "";
        } else {
            e.style.display = "none";
        }
        if (tipoVal.toUpperCase().indexOf(tipoMascota) > -1 || tipoVal == "All") {
            e.style.display = "";
        } else {
            e.style.display = "none";
        }
        if (sexoVal.toUpperCase().indexOf(razaMascota) > -1 || sexoVal == "All") {
            e.style.display = "";
        } else {
            e.style.display = "none";
        }
        if (razaVal.toUpperCase().indexOf(sexoMascota) > -1 || razaVal == "All") {
            e.style.display = "";
        } else {
            e.style.display = "none";
        }
...
function displayFilas(items, tbody, filasPorPagina, pagina) {

    tbody.innerHTML = "";
    pagina--;

    let inicio = filasPorPagina * pagina;
    let fin = inicio + filasPorPagina;
    let itemsPaginados = items.slice(inicio, fin);
    let fragment = document.createDocumentFragment();

    for (let i = 0; i < itemsPaginados.length; i++) {

        let item = itemsPaginados[i];
        let tr = document.createElement('tr');
...
function setearPaginacion(items, paginador, filasPorPagina) {

    paginador.innerHTML = "";
    let contadorPagina = Math.ceil(items.length / filasPorPagina);
    for (let i = 1; i < contadorPagina + 1; i++) {
        let btn = botonPaginacion(i, items);
        paginador.append(btn);
    }
}

function botonPaginacion(pagina, items) {
    ...
    button.addEventListener('click', () => {
        $('#tablaMascota tbody').html("");
        paginaActual = pagina;
        displayFilas(items, $('#tablaMascota tbody'), filasPorPagina, paginaActual);

        let botonActual = document.querySelector('#paginador button.active');
